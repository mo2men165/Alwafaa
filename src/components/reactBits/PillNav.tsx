...
const PillNav: React.FC<PillNavProps> = ({
  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach(circle => {
        ...
        tl.to(circle, { scale: 1.2, xPercent: -50, duration: 2.5, ease, overwrite: 'auto' }, 0);

        if (label) {
          tl.to(label, { y: -(h + 8), duration: 2.5, ease, overwrite: 'auto' }, 0);
        }

        if (white) {
          gsap.set(white, { y: Math.ceil(h + 100), opacity: 0 });
          tl.to(white, { y: 0, opacity: 1, duration: 2.5, ease, overwrite: 'auto' }, 0);
        }
  ...
  return (
    <div className="w-full pt-[1em] md:flex md:justify-center">
      <nav
